            ...
            ->children()
                ...
                ->arrayNode('totp')
                    ...
                    ->children()
                        ->booleanNode('enabled')->defaultFalse()->end()
                        ->scalarNode('issuer')->defaultNull()->end()
                        ->integerNode('period')->defaultValue(30)->min(1)->end()
                        ->integerNode('digits')->defaultValue(6)->min(1)->end()
                        ->scalarNode('digest')->defaultValue('sha1')->end()
                        ->arrayNode('parameters')
                            ->scalarPrototype()->end()
                        ->end()
                        ->scalarNode('qr_code_generator')->defaultValue('https://chart.googleapis.com/chart?chs=200x200&chld=M|0&cht=qr&chl={PROVISIONING_URI}')->end()
                        ->scalarNode('qr_code_data_placeholder')->defaultValue('{PROVISIONING_URI}')->end()
                        ->scalarNode('template')->defaultValue('@SchebTwoFactor/Authentication/form.html.twig')->end()
                    ->end()
                ->end()
